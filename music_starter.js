...
function draw_one_frame(words, vocal, drum, bass, other, counter) {
  ...
  textSize(24);
   ...
   let bassColor = map(bass, 0, 100, 0, 255);
   let bassColorB = map(bass, 0, 100, 0, 150);
   let bar_spacing = height / 10;
   let bar_height = width / 12;
   let bar_pos_x = width / 2;
 

   if (drum > 50 && bass > 50 && other > 50 && vocal < 20) {
    // Release special effects
    background(bassColor, 0, 0)
  } else {
    background(bassColorB, 0, 0);  // Set the background color based on the bass value
  }
   // vocal bar is red
   fill(200, 0, 0);
