function draw_one_frame(words, vocal, drum, bass, other, counter) {
  ...
  for (let i = 0; i <= 1; i += 0.07/other) {
    ...
    fill(inter);
    rect(0, i * height, width, height * 0.06/other);
  }
...
for (let i = 0; i < rectCount; i++) {
  ...
  // 使用 lerp 缓慢过渡矩形的高度
  let targetRectHeight = rectHeights[i];
  let smoothingFactor = 0.1; // 控制平滑度的因子
  rectHeights[i] = lerp(rectHeights[i], targetRectHeight, smoothingFactor);
  let rectHeight = rectHeights[i] + random(0, 100);
